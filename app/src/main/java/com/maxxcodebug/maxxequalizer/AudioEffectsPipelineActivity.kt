...
class AudioEffectsPipelineActivity : AppCompatActivity() {
    ...
    private fun currentAudioOutputDescription(): String? {
        ...
        for (d in am.getDevices(android.media.AudioManager.GET_DEVICES_OUTPUTS)) {
            ...
            com.maxxcodebug.maxxequalizer.audio.DeviceIdentity.keyOf(d) ?: continue
            val p = com.maxxcodebug.maxxequalizer.audio.DeviceIdentity.priority(d)
            if (p > bestPriority) {
                bestPriority = p
                best = d
            }
        ...
        val label = com.maxxcodebug.maxxequalizer.audio.DeviceIdentity.labelOf(active)
        val key = com.maxxcodebug.maxxequalizer.audio.DeviceIdentity.keyOf(active) ?: return label
        val keyDisplay = com.maxxcodebug.maxxequalizer.audio.DeviceIdentity.displayKey(key)
